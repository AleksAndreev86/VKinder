...
class VKBot:
    def __init__(self):
        ...
        self.vk = vk_api.VkApi(token=information.comm_token)
    ...
    def name(self, user_id):
        ...
        params = {'access_token': information.user_token,
                  'user_ids': user_id,
                  'v': '5.131'}
    ...
    def get_sex(self, user_id):
        """ПОЛУЧЕНИЕ ПОЛА ПОЛЬЗОВАТЕЛЯ, ПОЛ ОПРЕДЕЛЯЕТ ПОЛЬЗОВАТЕЛЬ"""
        url = f'https://api.vk.com/method/users.get'
        params = {'access_token': information.user_token,
                  'user_ids': user_id,
                  'fields': 'sex',
        ...
        try:
            ...
            for i in information_list:
                self.write_msg(user_id, 'Введите пол для поиска (женский - 1, мужской - 2): ')
                for event in self.longpoll.listen():
                    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                        ...
                        if any([gender == '1', gender == '2']):
                            ...
                            self.write_msg(user_id, 'Введено неверное значение для поиска. Перевведите.')
                            continue
        except KeyError:
            self.write_msg(user_id, 'Ошибка получения токена, введите токен в переменную - user_token')
    ...
    def send_photo_1(self, user_id, message, offset):
        ...
        self.vk.method('messages.send', {'user_id': user_id,
                                         'access_token': information.user_token,
                                         'message': message,
